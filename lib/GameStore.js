...
function GameStore() {
  ...
  setInterval(function(games) {
    for (var key in games) {
      if (Date.now() - games[key].modifiedOn > (1 * 60 * 60 * 1000)) {
        console.log("Deleting game " + key + ". No activity for atleast 1 hours.");
        delete games[key];
      }
    }
...
GameStore.prototype.add = function(gameParams, gameMode) {
  var key       = '';
  var keyLength = 7;
  ...
  this.games[key] = new Game(key,gameParams, gameMode);
